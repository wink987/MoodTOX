...
def _fallback_fragments(
    ...
    return set(
        sorted(fragments, key=len, reverse=True)[:max_fallback_fragments]
    )
...
def extract_substructures(
    ...
    if use_fallback:
        fragments = _fallback_fragments(
            mol, min_fragment_atoms, max_fallback_fragments
        )
    else:
        try:
